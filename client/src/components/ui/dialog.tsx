...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      'fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm data-[state=open]:animate-fade-in',
      className,
    )}
    {...props}
  />
...
const DialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & {
    hideClose?: boolean;
    /** Accessible name for dialogs that have no visible <DialogTitle>. */
    title?: string;
  }
  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        'border border-border bg-card p-6 shadow-xl duration-150 sm:rounded-2xl',
        'max-h-[92vh] overflow-y-auto data-[state=open]:animate-scale-in',
        className,
      )}
      {...props}
    >
